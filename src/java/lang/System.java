...
/**
    ...
    <p><code>System<code> 类提供的设施包括标准输入、标准输出和错误输出流；访问外部定义的属性和环境变量；
...
public final class System {
    ...
    /* 通过静态初始化程序注册本机。
     *
     * VM 会调用initializeSystemClass 方法来完成这个与clinit 分离的类的初始化。
     * 请注意，要使用 VM 设置的属性，请参阅 initializeSystemClass 方法中描述的约束。
     */
     ...
     * <p>
     调用 <code>System.loadLibrary(name)<code> 实际上等效于调用 <blockquote><pre> Runtime.getRuntime().loadLibrary(name) <pre><blockquote>
     *
