...
public class Arrays {
    ...
    /**
     * 可以使用系统属性选择旧的合并排序实现（为了与损坏的比较器兼容）。由于循环依赖，不能是封闭类中的静态布尔值。将在未来的版本中删除。
     */
     ...
     *  0.数组中的所有元素都必须实现 {@link Comparable} 接口。
     *  此外，数组中的所有元素必须
     *  1. 相互比较（即，{@code e1.compareTo(e2)} 不得为任何元素 {@code e1} 和 { @code e2} 在数组中）。
     *  2. 这种排序保证是稳定的：相同的元素不会因为排序而被重新排序
     *
     实现说明：此实现是一种稳定的、自适应的、迭代的归并排序，
     当输入数组部分排序时，它需要远少于 n 次 lg(n) 比较，
     而在输入数组随机排序时提供传统归并排序的性能.如果输入数组接近排序，则实现需要大约 n 次比较。
    ...
    public static <T,U> T[] copyOf(U[] original, int newLength, Class<? extends T[]> newType) {
        ...
        T[] copy = ((Object)newType == (Object)Object[].class)
            ? (T[]) new Object[newLength]
                // newType.getComponentType() 新数组的类型
