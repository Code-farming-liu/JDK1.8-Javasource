...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     * DEFAULT_CAPACITY.
     */
    //每个传输步骤的最小重新绑定数量。范围*被细分为允许多个调整程序线程。
    // *此值用作下限，以避免调整器遇到*过多的内存争用。
    // 该值至少应为* DEFAULT_CAPACITY。
    ...
    public ConcurrentHashMap(int initialCapacity) {
        ...
        //如果初始化容量 大于等于 2 ^ 22 返回最大值 否则返回2的整数幂次
     ...
     * @param m the map
     */
    //当输入一个Map的时候，先设定sizeCtl为默认容量，在添加元素
     ...
     * 如果取出来的节点的hash值是MOVED(-1)的话，
     * 则表示当前正在对这个数组进行扩容，复制到新的数组，则当前线程也去帮助复制
     * 最后一种情况就是，如果这个节点，不为空，也不在扩容，
     * 则通过synchronized来加锁，进行添加操作
     *    然后判断当前取出的节点位置存放的是链表还是树
     *    如果是链表的话，则遍历整个链表，
     * 直到取出来的节点的key来个要放的key进行比较，如果key相等，并且key的hash值也相等的话，
     *          则说明是同一个key，则覆盖掉value，
     * 否则的话则添加到链表的末尾
     *    如果是树的话，则调用putTreeVal方法把这个元素添加到树中去
     *  最后在添加完成之后，
     * 会判断在该节点处共有多少个节点（注意是添加前的个数），如果达到8个以上了的话，
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            ...
            if ((sc = sizeCtl) < 0)
                ...
                // 设定为-1表示要初始化表了
            //cas 原理
                        //sc 大于 0 如果sizeCtl大于0的话，则初始化一个大小为sizeCtl的数组
     ...
     * 计算出来c的值为64,则要扩容到sizeCtl 大于等于为止
     ...
     * 每个CPU最少处理16个长度的数组元素,也就是说，
     * 如果一个数组的长度只有16，那只有一个线程会对其进行扩容的复制移动操作
     * 扩容的时候会一直遍历，知道复制完所有节点，
     * 没处理一个节点的时候会在链表的头部设置一个fwd节点，这样其他线程就会跳过他，
     * 复制后在新数组中的链表不是绝对的反序的
     */
                        ...
                        if (fh >= 0) {
                            ...
                            for (Node<K,V> p = f; p != lastRun; p = p.next) {
                                ...
                                if ((ph & n) == 0)
                                    /*
                                     * 假设runBit的值为0，
                                     * 则第一次进入这个设置的时候相当于把旧的序列的
                                     * 最后一次发生hash变化的节点(该节点后面可能还有hash计算后同为0的节点)
                                     ...
                                     * 则第一次进入这个设置的时候相当于把旧的序列的
                                     * 最后一次发生hash变化的节点
                                     * (该节点后面可能还有hash计算后同不为0的节点)设置到旧的table的
                                     * 第一个hash计算后不为0的节点下一个节点
                                     * 并且把自己返回，然后在下次进来的时候把它自己设置为后面节点的下一个节点
                                     */
